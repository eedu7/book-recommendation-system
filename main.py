...
raw_document = TextLoader("tagged_description.txt").load()  # Reading tag description
text_splitter = CharacterTextSplitter(separator="\n", chunk_size=0, chunk_overlap=0) # Character text splitter
documents = text_splitter.split_documents(raw_document)  # Applying to each docs
# Document embedding into chroma vector database
db_books = Chroma.from_documents(documents, embedding=GoogleGenerativeAIEmbeddings(model="models/embedding-001", google_api_key=config.GOOGLE_API_KEY))
...
def retrieve_semantic_recommendations(
    ...
    if category != "All":
        books_recs = books_recs[books_recs["simple_characters"] == category][:final_top_k]
    else:
